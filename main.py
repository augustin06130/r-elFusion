...
from config import parse_arguments, CLIENT_ID_REDDIT, CLIENT_SECRET_REDDIT
from reddit_client import setup_reddit, get_reddit_content
from text_processor import process_text_for_tts, split_text_into_chunks, setup_nltk
from translator import translate_text_with_gpt, detect_language
from media_generator import process_video_from_text
from youtube_dl import download_video_by_theme
...
def main():
    ...
    try:
        ...
        # Nettoyer et optimiser le texte pour TTS
        logger.info("Nettoyage et optimisation du texte pour TTS")
        try:
            # Utiliser la nouvelle fonction qui nettoie ET optimise pour TTS
            cleaned = process_text_for_tts(
                reddit_text,
                openai_client=client,
                model=args.gpt_model,
                optimize_for_tts=True  # Activer l'optimisation TTS
            )
            logger.debug(f"Texte traité: {len(cleaned)} caractères")
        except Exception as e:
            logger.warning(f"Erreur lors du traitement du texte: {str(e)}")
            logger.warning("Utilisation du nettoyage simple sans optimisation TTS")
            # Fallback sur le nettoyage simple
            from text_processor import clean_reddit_text
            cleaned = clean_reddit_text(reddit_text)

        try:
            source_language = detect_language(cleaned, client, args.gpt_model)
            logger.debug(f"Langue détectée: {source_language}")
          ...
          # Traduire le texte avec GPT
            if source_language != args.target_language:
                ...
                try:
                    ...
                    # Optimiser le texte traduit pour TTS si nécessaire
                    logger.info("Optimisation du texte traduit pour TTS")
                    try:
                        from text_processor import optimize_text_for_tts
                        translated = optimize_text_for_tts(translated, client, args.gpt_model)
                    except Exception as e:
                        logger.warning(f"Erreur lors de l'optimisation TTS du texte traduit: {e}")
                        # Continuer avec le texte traduit non optimisé
